App.namespace 'App.Views.Forum.Boards', (ns)->
        class ns.New extends Backbone.View
                ...
                initialize: (options) ->
                        @groupPreview=new App.Views.Forum.Groups.Show
                                model:new App.Models.BoardGroup()
                        @boardPreview=new App.Views.Forum.Boards.Show
                                ...
                                when 'keyup'
                                        switch event.keyCode
                                                ...
                                                when 27
                                                        ...
                                                        @groupPreview.model.set
                                                                name:@$el.find('.new-inputs input#board-group').val()
                create:->
                        @trigger 'collection.add', new App.Models.Board
                                name:@$el.find('.new-inputs input#board-name').val()
                                description:@$el.find('.new-inputs input#board-description').val()
                                board_group:
